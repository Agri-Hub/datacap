...
def time_parse(timestr):
    ...
    aos = timestr.split('_')[0][:4]+'-'+timestr.split('_')[0][4:6]+'-'+timestr.split('_')[0][6:8]
    los = timestr.split('_')[1][:4]+'-'+timestr.split('_')[1][4:6]+'-'+timestr.split('_')[1][6:8]
    return aos, los
...
def prep_dataset(fields, path):
    docs = []
    # print('Examing path:',path)
    for tif in os.listdir(path):
        if not tif.endswith('.tif'):
            continue
        if '_VV_coherence_3857' in tif:
            continue

        aos, los = time_parse(tif)

        fields['creation_dt'] = aos
        fields['satellite'] = 'sentinel1'
        images = {band_name(im_path): {
            'path': str(im_path.relative_to(path))
        } for im_path in path.glob(tif.split('VH_coherence_3857')[0]+'*.tif')}
        print("Checking:",tif)
        first_image = list(images.values())[0]['path'][:-4]
        first_image = first_image.replace('_VV_coherence_3857','')
        first_image = first_image.replace('_VH_coherence_3857','')
        print(first_image)

        doc = {
            'id': str(uuid.uuid4()),
            # Strips the path of the suffix and generating a name for the generated yaml
            'name': first_image,# list(images.values())[0]['path'][:-4].replace('VV', '').replace('VH', ''),
...
def prepare_datasets(s1a_path):
    ...
    fields = re.match((r"(?P<platform>coherence)"), s1a_path.stem).groupdict()
...
@click.command(help="Prepare sentinel1 inSAR dataset for ingestion into the Data Cube.")
...
def main(datasets):
    ...
    for y in os.listdir(path):
        if not y.endswith('.yaml'):
            continue
        print("Adding...",y)
        ws_file = os.path.join(path,y)
