...
class Mapfile(MyLogger):
    ...
    def _add_map(self, in_rasters, in_vectors):
        ...
        base_data = {'xmin': self.extent[0], 'ymin': self.extent[1], 'xmax': self.extent[2], 'ymax': self.extent[3],
                     'url': 'http://' + get_ip_address() + '/cgi-bin/mapserv',
                     'path_mapfile': os.path.join(self.base_dir, 'wms.map'), 'rasters_lyr': in_rasters,
                     'vectors_lyr': in_vectors}
    ...
    def _get_path_tifs(self):
        ...
        self.logger.info('Raster\'s pathname of the directory %s have been successfully initialized',
                         os.path.split(self.base_dir)[1])
        return rasters
    ...
    def _get_path_shp(self):
        ...
        self.logger.info('Vector\'s pathname of the directory %s have been successfully initialized',
                         os.path.split(self.base_dir)[1])
        return vectors
...
class MapProxy(MyLogger):
    ...
    def _get_wms_layers(self):
        ...
        layer_name = [child.getchildren()[0].text for i, child in
                      enumerate(wms_xml.iter('{http://www.opengis.net/wms}Layer')) if i > 0]
        logging.info(layer_name)
        layer_bbox = [
            [float(child.getchildren()[0].text), float(child.getchildren()[2].text), float(child.getchildren()[1].text),
             float(child.getchildren()[3].text)] for i, child in
            enumerate(wms_xml.iter('{http://www.opengis.net/wms}EX_GeographicBoundingBox')) if i > 0]
...
class Sentinel2_pre_process(MyLogger):
    ...
    prefixes = [('B02_10m', 'blue'), ('B03_10m', 'green'), ('B04_10m', 'red'), ('B05_20m', 'vre1'), ('B06_20m', 'vre2'),
                ('B07_20m', 'vre3'), ('B08_10m', 'nir1'), ('B8A_20m', 'nir2'), ('B11_20m', 'swir1'),
                ('B12_20m', 'swir2'), ('SCL_20m', 'cloud'), ('TCI_10m', 'tci')]
    ...
    def converting_rasters(self):
        ...
        self.logger.info('Resampling and Transformation of 20_m and 10_m bands of %s, respectively, just finished.',
                         self.product)
    ...
    def raster2geotiff(img_dir, in_ras_file, out_ras_file):
        ...
        if '_TCI' in in_ras_file:
            ...
            in_band = in_ras.GetRasterBand(1)
            driver = gdal.GetDriverByName('GTiff')
            out_ds = driver.Create(os.path.join(img_dir, out_ras_file), in_ras.RasterXSize, in_ras.RasterYSize, 1,
                                   in_band.DataType)
            out_ds.SetProjection(in_ras.GetProjection())
    ...
    def _get_path_bands(self):
        ...
        try:
            ...
            self.logger.info('Directory pathname for the specified bands of %s have been successfully initialized',
                             self.product)
            return bands
...
class Sentinel2_process(MyLogger):
    ...
    prefixes = {'B02': 'blue', 'B03': 'green', 'B04': 'red', 'B05': 'vre1', 'B06': 'vre2', 'B07': 'vre3', 'B08': 'nir',
                'B8A': 'nir_vre', 'B11': 'swir1', 'B12': 'swir2', 'SCL': 'cloud', 'RGB': 'tci'}
    ...
    def maskcloud(self):
        ...
        for x in range(0, xsize, block_xsize):
            ...
            # self.bands[item])[1].split('_')[1]), out_data, gdal.GDT_Float16, -999999)

        out_ndvi = self.create_raster(gdal.Open(self.bands['cloud']), os.path.join(self.ws,
                                                                                   os.path.split(self.bands['cloud'])[
                                                                                       1].split('_')[
                                                                                       0] + '_CloudMask.tif'), out_data,
                                      gdal.GDT_Int16, -999999)
    ...
    def maskcloudphenology(self):
        ...
        out_ndvi = self.create_raster(gdal.Open(self.bands['cloud']), os.path.join(self.ws,
                                      ...
                                      out_data, gdal.GDT_Int16, -999999)
    ...
    def shadow(self):
        ...
        out_ndvi = self.create_raster(gdal.Open(self.bands['cloud']), os.path.join(self.ws, os.path.split(
            self.bands['cloud'])[1].split('_')[0] + '_Shadows.tif'), out_data, gdal.GDT_Float16, -999999)
    ...
    def create_ndvi(self):
        ...
        out_ndvi = self.create_raster(gdal.Open(self.bands['red']), os.path.join(self.ws,
                                                                                 os.path.split(self.bands['red'])[
                                                                                     1].split('_')[
                                                                                     0] + '_NDVI_INDEX.tif'), out_data,
                                      gdal.GDT_Int16, -999999)
    ...
    def create_ndvi_lth(self):
        ...
        check = np.logical_or(red > 0, nir > 0)
        # red = np.ma.masked_where(nir + red == 0, red)
        ndvi = np.where(check, (nir - red) / (nir + red) * 1000, -999999)
        del check
        # ndvi = (nir - red) / (nir + red) * 1000
        # ndvi = ndvi.filled(-999999)
        del red
        ...
        for x in range(0, xsize, block_xsize):
            ...
            for y in range(0, ysize, block_ysize):
                ...
                out_data[y:y+rows, x:cols] = ndvi
        '''
        # out_ndvi = self.create_raster(gdal.Open(self.bands['red']), os.path.join(self.ws, os.path.split(self.bands['red'])[1].split('_')[0] + '_NDVI_INDEX.tif'), out_data, gdal.GDT_Float16, -999999)
    ...
    def create_bsm(self):
        ...
        del nir_band
        check = np.logical_or(red > 0, nir > 0)
        ndvi = np.where(check, (nir - red) / (nir + red), -999999)
        del check
        del red
        del nir
        ...
        ndwi = np.where(check, (green - nir) / (green + nir), -999999)
        del check
        del nir
        del green
        ...
        del swir2_band
        check = swir2 > 0
        lidx = np.where(check, swir / swir2, -999999)
        del check
        del swir2
        del swir
        ...
        nbr = np.where(check, (nir - swir2) / (nir + swir2), -999999)
        del check
        del swir2
        del nir
        ...
        std1 = np.std(nbr[nbr > -999999])
        nbr = nbr[nbr > -999999] / std1
        std2 = np.std(ndvi[ndvi > -999999])
        ndvi = ndvi[ndvi > -999999] / std2
        std3 = np.std(ndwi[ndwi > -999999])
        ndwi = ndwi[ndwi > -999999] / std3
        std4 = np.std(lidx[lidx > -999999])
        lidx = lidx[lidx > -999999] / std4
        ...
        driver = gdal.GetDriverByName("GTiff")
        dst_ds = driver.Create(os.path.join(self.ws, os.path.split(self.bands['red'])[1].split('_')[0] + '_bsm.tif'),
                               shape[1], shape[0], 1, gdal.GDT_Float16)
        dst_ds.SetGeoTransform(geo)
        dst_ds.SetProjection(proj)
    ...
    def create_psri_lth(self):
        ...
        vre1_ds = gdal.Open(self.bands['vre1'])
        ...
        check = vre1 > 0
        # red = np.ma.masked_where(nir + red == 0, red)
        psri = np.where(check, (red - blue) / vre1 * 1000, -999999)
        del check
        del red
        del blue
        del vre1
        ...
        driver = gdal.GetDriverByName("GTiff")
        dst_ds = driver.Create(
            os.path.join(self.ws, os.path.split(self.bands['red'])[1].split('_')[0] + '_PSRI_INDEX.tif'), shape[1],
            shape[0], -999999, gdal.GDT_Float16)
        dst_ds.SetGeoTransform(geo)
        dst_ds.SetProjection(proj)
    ...
    def create_psri(self):
        ...
        out_psri = self.create_raster(gdal.Open(self.bands['red']), os.path.join(self.ws,
                                                                                 os.path.split(self.bands['red'])[
                                                                                     1].split('_')[
                                                                                     0] + '_PSRI_INDEX.tif'), out_data,
                                      gdal.GDT_Int16, -999999)
    ...
    def create_ndwi_lth(self):
        ...
        ndwi = np.where(check, (nir - green) / (green + nir) * 1000, -999999)
    ...
    def create_ndwi(self):
        ...
        out_ndwi = self.create_raster(gdal.Open(self.bands['green']), os.path.join(self.ws,
                                                                                   os.path.split(self.bands['green'])[
                                                                                       1].split('_')[
                                                                                       0] + '_NDWI_INDEX.tif'),
                                      out_data, gdal.GDT_Int16, -999999)
    ...
    def create_savi_lth(self):
        ...
        # nir = np.ma.masked_where(green + nir == 0, nir)
        savi = np.where(check, (1.5 * (nir - red)) / (nir + red + 0.5), -999999)

        # savi = (1.5 * (nir - red)) / (nir + red + 0.5)
        # savi = savi.filled(-999999)
        del check
        del red
        del nir
    ...
    def create_savi(self):
        ...
        out_savi = self.create_raster(gdal.Open(self.bands['red']), os.path.join(self.ws,
                                                                                 os.path.split(self.bands['red'])[
                                                                                     1].split('_')[
                                                                                     0] + '_SAVI_INDEX.tif'), out_data,
                                      gdal.GDT_Float32, -999999)
    ...
    def create_nbr(self):
        ...
        for x in range(0, xsize, block_xsize):
            ...
            for y in range(0, ysize, block_ysize):
                ...
                nbr = nbr.filled(-999999)

                out_data[y:y + rows, x:cols] = nbr

        out_nbr = self.create_raster(gdal.Open(self.bands['nir']), os.path.join(self.ws,
                                                                                os.path.split(self.bands['nir'])[
                                                                                    1].split('_')[
                                                                                    0] + '_NBR_INDEX.tif'), out_data,
                                     gdal.GDT_Float16, -999999)
    ...
    def create_rgb_8bit(self):
        ...
        out_ds = red_ds.GetDriver().Create(
            os.path.join(self.ws, os.path.split(self.bands['red'])[1].split('_')[0] + '_RGB.tif'), red_ds.RasterXSize,
            red_ds.RasterYSize, 3, red_band.DataType, options=['COMPRESS=LZW', 'TILED=YES', 'BIGTIFF=YES'])
        ...
        self.logger.info('Production of RGB raster of %s has been successfully fnished',
                         os.path.split(self.bands['red'])[1].split('_')[0] + '_RGB.tif')
    ...
    def convert_to_8bit(in_ras_file):
        ...
        out_ds = ras_ds.GetDriver().Create(
            os.path.join(os.path.split(in_ras_file)[0], os.path.split(in_ras_file)[1].split('.')[0] + '_8bit.tif'),
            ras_ds.RasterXSize, ras_ds.RasterYSize, 1, gdal.gdalconst.GDT_Byte)
        ...
        for parcel in Parcels.objects.filter(aoi_id=in_aoi_id):
            ...
            for nband in range(ras.RasterCount):
                try:
                    ...
                    zone_ras = np.ma.masked_array(data_band, np.logical_not(mask), fill_value=-999999)
    ...
    for band in range(ras.RasterCount):
        ...
        for parcel in Parcels.objects.filter(aoi_id=in_aoi_id):
            ...
            try:
                ...
                cc = off_ulx
                dd = off_lrx + 1
                zone_ras = np.ma.masked_array(values[aa:bb, cc:dd], np.logical_not(mask), fill_value=-999999)
                zone_ras_list = zone_ras.compressed().tolist()
                # parcel_data.append(np.mean(zone_ras_list))
...
def get_token(in_url='https://app.recap-h2020.eu/backend/api/authentication/login/',
              in_data={"username": "alelentzis", "email": "alelentzis@noa.gr", "password": "r3c@pNoa"}):
...
def reproject(ws):
    for product in os.listdir(ws):
        if not product.endswith('tif'):
            continue
        new_product = os.path.join(ws,product.split('.')[0]+'_3857.tif')
        if os.path.exists(new_product):
            continue
        old_product = os.path.join(ws,product)
        gdal.Warp(new_product,old_product,dstSRS='EPSG:3857',xRes=10,yRes=10)
